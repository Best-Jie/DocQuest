...
class imageRetrieval(BaseRetrieval):
    def __init__(self, config):
        ...
        acc = Accelerator()
        self.model, self.processor = acc.prepare(self.model, self.processor)
    ...
    def find_top_k_batch(self, dataset: BaseDataset, start_idx: int, end_idx: int):
        """
        该函数用于分批次处理样本，避免一次性加载过多数据导致显存溢出。

        :param dataset: 数据集实例
        :param start_idx: 开始样本索引
        :param end_idx: 结束样本索引（不包含）
        """
        batch_size = 20
        for batch_start in range(start_idx, end_idx, batch_size):
            batch_end = min(batch_start + batch_size, end_idx)
            samples = dataset.load_data(use_retrieval=True)[batch_start:batch_end]

            document_embeds = self.prepare(dataset)  # 强制prepare，重新生成embeddings
            ...
            for sample in tqdm(samples):
                ...
                try:
                    top_page_indices, top_page_scores = self.find_sample_top_k(
                        sample, document_embed, self.top_k, dataset.page_id_key
                    )
                except torch.OutOfMemoryError as e:
                    torch.cuda.empty_cache()
                    continue
